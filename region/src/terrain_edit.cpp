...
float neighbor_average(const Heightmap& source, int x, int y) {
    ...
    for (int dy = -1; dy <= 1; ++dy)
        for (int dx = -1; dx <= 1; ++dx) {
            const auto sample_x = std::clamp(x + dx, 0, 255);
            const auto sample_y = std::clamp(y + dy, 0, 255);
            total += source[static_cast<std::size_t>(sample_y) * 256 + sample_x];
            ++count;
        }
...
std::unique_ptr<Heightmap> load_state(const std::filesystem::path& path) {
    ...
    if (!input || input.tellg() != static_cast<std::streamoff>(sizeof(Heightmap))) return {};
    input.seekg(0);
    auto result = std::make_unique<Heightmap>();
    input.read(reinterpret_cast<char*>(result->data()), sizeof(Heightmap));
...
bool save_state(const std::filesystem::path& path, const Heightmap& heightmap) {
    ...
    const auto temporary = path.string() + ".tmp";
    {
        ...
        output.write(reinterpret_cast<const char*>(heightmap.data()), sizeof(heightmap));
        if (!output) return false;
    }
...
std::vector<viewer::TerrainPatch> apply(Heightmap& heightmap, const Heightmap& revert,
    ...
    for (std::size_t area_index = 0; area_index < edit.areas.size(); ++area_index) {
        ...
        const auto x_from = std::clamp(static_cast<int>(std::floor(center_x - radius)), 0, 255);
        const auto x_to = std::clamp(static_cast<int>(std::ceil(center_x + radius)), 0, 255);
        const auto y_from = std::clamp(static_cast<int>(std::floor(center_y - radius)), 0, 255);
        const auto y_to = std::clamp(static_cast<int>(std::ceil(center_y + radius)), 0, 255);
        ...
        for (int y = y_from; y <= y_to; ++y)
            for (int x = x_from; x <= x_to; ++x) {
                ...
                const auto weight = std::max(0.0F, std::cos(distance * std::numbers::pi_v<float> /
                                                            (radius * 2.0F)));
                const auto index = static_cast<std::size_t>(y) * 256 + x;
