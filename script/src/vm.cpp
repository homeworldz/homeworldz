...
VM::VM(const Program& program) : program_(program) {
    locals_.assign(static_cast<std::size_t>(program_.local_count), Value::make_integer(0));
}
...
void VM::step() {
    ...
    const auto local_ref = [this](std::int32_t slot) -> Value& {
        if (slot < 0 || static_cast<std::size_t>(slot) >= locals_.size()) {
            throw VmFault("local slot out of range");
        }
        return locals_[static_cast<std::size_t>(slot)];
    };
    ...
    case Op::LoadLocal:
        stack_.push_back(local_ref(instr.a));
        break;
    case Op::StoreLocal:
        local_ref(instr.a) = pop();
        break;
    case Op::AddInt: {
...
std::vector<std::uint8_t> VM::snapshot() const {
    ...
    w.u32(static_cast<std::uint32_t>(locals_.size()));
...
void VM::restore(const std::vector<std::uint8_t>& bytes) {
    ...
    for (std::uint32_t i = 0; i < stack_size; ++i) {
        stack_.push_back(r.value());
    }

    const std::uint32_t locals_size = r.u32();
    if (locals_size != static_cast<std::uint32_t>(program_.local_count)) {
        throw ScriptError("snapshot local count does not match this program");
    }
    locals_.clear();
    locals_.reserve(locals_size);
    for (std::uint32_t i = 0; i < locals_size; ++i) {
        locals_.push_back(r.value());
    }
}
