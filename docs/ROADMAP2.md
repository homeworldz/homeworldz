...
- [x] Choose C++ with an engine-neutral core, reject Unreal on license grounds
  ...
  (ADR 0030).
- [x] Adopt SLua compatibility as the expected direction for Lua scripting,
  superseding an earlier plan for a HomeWorldz-specific subset and p-code
  machine, and record the gates that would reverse it (ADR 0031).
...
The current expectation is to **adopt SLua** — Linden Lab's MIT-licensed fork of
Luau, which already implements execution-state serialization (Ares), preemptive
scheduling hooks, per-script memory limits, and sandboxing — rather than build a
second p-code machine. That expectation is contingent on the evaluation
milestone below, and the fallback in ADR 0031 applies if it fails.
...
- [ ] Record the adopt-or-build outcome in ADR 0031, resolving its verification
  gates one way or the other.
...
- [ ] Inventory the divergences between the HomeWorldz `ll` surface and Second
  Life's, and document each as an intentional difference.
