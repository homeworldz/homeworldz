...
| `region.walkable_slope_degrees` | Steepest ground an avatar can **walk up or along** (10–89) — Jolt's `CharacterVirtual` max slope angle. **It does not bound resting.** `grounded` is a contact test, true on any slope, and no slide is implemented, so an avatar rests on ground far steeper than this and simply cannot traverse it. The row previously read "steepest ground an avatar stands on … so avatars slide"; both halves were false and a client built a resting prediction on them, corrected 2026-08-05 when three fixture faces showed an avatar resting motionless on 70° and unable to walk up 65°. Published to session clients in the hello, with `slopeLimitGoverns` and `restingBoundedBySlope` beside it so the scope is stated rather than inferred | `65` (Halcyon's limit; the *walking* was confirmed in-world 2026-07-30 — the resting claim never was) |
