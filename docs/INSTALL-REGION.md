...
| `region.water_height` | Height in metres of the region's flat, region-wide water plane, in the same vertical datum as terrain heights (0–4096). Viewers read it from `RegionHandshake`, session clients from the hello's `water` block; before this was configurable both got the built-in 20 | `20` |
| `region.terrain_blend_tenths` | Width of the transition between two terrain layers, in **tenths of a metre**, straddling each boundary symmetrically — 20 means one metre either side (0–200). **Advisory:** only a client that shades its own terrain honours it. No legacy message carries a blend width, so Firestorm computes its own transition — including a noise term that was never reproduced outside Linden — and this cannot narrow it | `20` (2 m total) |
